...
func TestLoggerWritesChineseMessageToStdoutAndDailyFile(t *testing.T) {
	...
	assertConsoleLineContains(t, stdout.String(), []string{
		"2026-04-24 12:00:00 [INFO]",
		"开始执行目录检测",
	})
	if strings.Contains(stdout.String(), "event=scan_start") {
		t.Fatalf("stdout contains structured event field: %q", stdout.String())
	}
	...
	assertStructuredLineContains(t, string(data), []string{
		"2026-04-24 12:00:00 [INFO]",
		"event=scan_start",
		"开始执行目录检测",
	})
...
func TestLoggerFallsBackToStdoutWhenFileSetupFails(t *testing.T) {
	...
	if strings.Contains(stdout.String(), "event=file_setup_failed") || strings.Contains(stdout.String(), "attempt=1") {
		t.Fatalf("stdout contains structured fields after file setup failure: %q", stdout.String())
	}
}
...
func TestLoggerRotatesAcrossMidnight(t *testing.T) {
	...
	assertStructuredLineContains(t, firstDay, []string{"event=before_midnight", "before"})
	...
	assertStructuredLineContains(t, secondDay, []string{"event=after_midnight", "after"})
...
func TestLoggerClosesFileWhenRetentionCleanupFails(t *testing.T) {
	...
	assertConsoleLineContains(t, stdout.String(), []string{"stdout only"})
	if strings.Contains(stdout.String(), "event=after_cleanup_error") {
		t.Fatalf("stdout contains structured event after cleanup failure: %q", stdout.String())
	}
...
func TestLoggerEscapesUnsafeFieldValuesIntoOneLine(t *testing.T) {
	...
	got := readLogFile(t, dir, "2026-04-24.log")
	if strings.Count(got, "\n") != 1 {
		t.Fatalf("log output has %d newlines, want exactly 1: %q", strings.Count(got, "\n"), got)
	}
	assertStructuredLineContains(t, got, []string{
		"path=\"/tmp/media file\\nnext \\\"quote\\\" \\\\slash\"",
		"attempt=2",
		"err=\"bad file\\nsecond line\"",
	})
...
func TestLoggerSanitizesUnsafeFieldKeys(t *testing.T) {
	...
	if strings.Contains(got, "bad key") || strings.Contains(got, "with=equals") {
		t.Fatalf("log output contains unsanitized key: %q", got)
	}
}
...
func assertConsoleLineContains(t *testing.T, line string, wantParts []string) {
	t.Helper()
	...
	if !regexp.MustCompile(`^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} \[(INFO|ERROR)\] .+`).MatchString(line) {
		t.Fatalf("log line %q does not have expected prefix format", line)
	}
}

func assertStructuredLineContains(t *testing.T, line string, wantParts []string) {
	t.Helper()
	if !strings.HasSuffix(line, "\n") {
		t.Fatalf("log line %q missing trailing newline", line)
	}
